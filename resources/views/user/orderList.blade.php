                            ...
                            <table class="table table-hover table-light table-checkable order-column">
                                <thead>
                                    <tr>
                                        ...
                                        <th> {{trans('home.invoice_able_days')}} </th>
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td>
                                                @if(!$order->is_expire)
                                                    ...
                                                    @elseif($order->status == 0)
                                                        ...
                                                        @if(!empty($order->payment))
                                                            <a href="{{url('payment/' . $order->payment->sn)}}{{'/'.$order->p_type}}" target="_self" class="btn btn-sm red">{{trans('home.pay')}}</a>
                                                        @endif
                                                ...
                                                @else
                                                    <a href="javascript:;" class="btn btn-sm default disabled"> {{trans('home.invoice_table_expired')}} </a>
                                                @endif
                                                    <a href="{{url('/user/addOrder?goods_id=' . $order->goods->id)}}" target="_self" class="btn btn-sm green">{{trans('home.invoice_pay_more')}}</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endif
