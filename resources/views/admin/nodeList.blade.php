                                    ...
                                    @else
                                        @foreach($nodeList as $node)
                                            <tr class="odd gradeX">
                                                ...
                                                <td> {{$node->transfer}} / {{$node->traffic}} GB</td>
                                                ...
                                                <td>
                                                    ...
                                                    @if(!$node->is_build)
                                                        <button type="button" class="btn btn-sm green btn-outline" onclick="installNode('{{$node->id}}','{{$node->traffic_rate}}','{{$node->ip}}','{{$node->root}}','{{$node->pwd}}','{{$node->serverport}}')">
                                                            <i class="fa fa-code"></i>
                                                        </button>
                                                    @else
                                                        <button type="button" class="btn btn-sm green btn-outline" disabled>
                                                            <i class="fa fa-code"></i>
                                                        </button>
                                                    @endif
                                                </td>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        //安装脚本
        function installNode(nid,trans,ip,root,pwd,port) {
            layer.confirm('确定要安装SSR脚本么？（请先确保节点服务器中加入后台服务器公钥）', {icon: 3, title:'警告'}, function(index) {
                layer.load(1);
                $.post("{{url('wSifGFeO5mQoCWB4/installNode')}}", {
                    _token:'{{csrf_token()}}',
                    n_id:nid,
                    n_transfer:trans,
                    n_ip:ip,
                    ...
                    if (ret.status == 0) {
                        layer.alert('一键部署成功：'+ret.data, {icon: 1});
                    }else{
                        layer.alert('失败：'+ret.data, {icon: 2});
                    }
