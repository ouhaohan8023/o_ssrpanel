    ...
    public function active_profile(Request $request, $token)
  {
    ...
    public function resetPassword(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            if ($user->u_phone_status){
              $data['c_phone'] = $username;
              if (empty($data['c_phone'])) {
                $request->session()->flash('successMsg', '请输入手机号');
                return Redirect::back();
              }
              //同一个号码一小时5次限制
              ...
              if($ret){
                ...
                $verify->status = 0;
                $verify->save();
                $request->session()->flash('successMsg', '验证码发送成功');
              }else{
                $request->session()->flash('errorMsg', '验证码发送失败');
              }
              $view['website_analytics'] = self::$config['website_analytics'];
              $view['website_customer_service'] = self::$config['website_customer_service'];
              $view['is_reset_password'] = self::$config['is_reset_password'];
              $view['phone'] = $username;
              return Response::view('user/resetPasswordByPhone', $view);
            }else{
              // 生成取回密码的地址
              $token = md5(self::$config['website_name'] . $username . microtime());
              $verify = new Verify();
    ...
    public function resetPasswordByPhone(Request $request)
    {
      if ($request->method() == 'POST') {
        ...
        $smsData = \DB::table('smscode')->where('c_phone','=',$phone)->orderBy('c_id','DESC')->first();
        if(strtolower($code) != $smsData->c_code){
          $request->session()->flash('errorMsg', '短信验证码错误，请重新接收验证码。一天只能申请5条验证码哟！');
          return redirect('resetPassword');
        }else{
          $token = \DB::table('verify')->where('username','=',$phone)->orderBy('id','DESC')->first();
          $resetPasswordUrl = self::$config['website_url'] . '/reset/' . $token->token;
    ...
    public function download() {
      $view['website_analytics'] = self::$config['website_analytics'];
      $view['website_customer_service'] = self::$config['website_customer_service'];
      return Response::view('user/download',$view);
    }
